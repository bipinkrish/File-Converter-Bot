...
# suporrted extension
VIDAUD = ("AIFF", "AAC", "M4A", "OGA", "WMA", "FLAC", "WAV", "OPUS", "OGG", "MP3",
          "MKV", "MP4", "MOV", "AVI", "M4B", "VOB", "DVD", "WEBM", "WMV")
...
def follow(message, inputt, new):
    ...
    if output.upper().endswith(VIDAUD) and inputt.upper().endswith(VIDAUD):
        ...
        if new == "ico":
            ...
            for ele in slist:
                toutput = update_file(inputt, f"{ele}.png")
                os.remove(toutput)
        task = task - 1
        os.remove(file)
        ...
        file = app.download_media(message)
        cmd = calibre_command(file, output)
        os.system(cmd)
        os.remove(file)
        try:
            app.send_document(message.chat.id, document=output)
